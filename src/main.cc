...
//	PARTICLE ANALYSIS
	...
	if( D > B ) 
	{
		cl4dg.set_matrix(coord_space);
	}
	else
	{
		cl4dg.set_matrix(carth_space);
	}
	cl4dg.print_all();
	...
	for (int j = 0; j < MM; j++)
	{
		...
		std::cout << "INFO::STD::" << sqrt(gsl_vector_get(sig,j)/gsl_vector_get(cnt,j)) << " " << gsl_vector_get(cnt,j) << std::endl;
	}
	std::cout << "INFO::ER::"<< Er/((float)MM) << std::endl;
	...
	{
		for(int i=0;i<MM;i++)
			new_parts[i].first = carth_space[i].first ;
	}
	else
	{
		for(int i=0;i<MM;i++)
			new_parts[i].first = coord_space[i].first ;
	}
