...
//	cl4dg.print_all();
	...
	for(int i=0;i<MM;i++)
		new_parts[i].first = model[i].first; // vparts[1][i].first ;
	
	richanalysis::cluster clnew;
	clnew.set_matrix(new_parts);
	clnew.find_centroids();
	richanalysis::node	n1;
	n1.first	= n0.second;
	n1.second	= clnew;
//	n1.first	= clnew;
//	n1.second	= n0.second;
	richanalysis::node_analysis nnode1(n1);
	particles new_aligned = nnode.regular_fit();
	for( int i = 0 ; i < new_aligned.size() ; i++ )
		new_aligned[i].first = model[i].first; 
	std::vector<int> labels = n1.first.get_labels();
	std::cout << "::INFO::LABEL::SIZE::" << labels.size() << std::endl;
	fIO.output_pdb("nrn1" + opts[2].second , new_aligned , labels);
*/
	return 0;
}
