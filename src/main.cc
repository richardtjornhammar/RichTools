...
//
	...
	if( L[0] >= L[1] ) {
		...
		n1.first.set_matrix ( vparts[0] ); densi=vparts[0];
		n1.second.set_matrix( vparts[1] ); model=vparts[1];
	} else { 
	...
	n1.second.set_cDIM(s);	
	n1.second.realloc_centroids(s);
	std::cout << "WILL ORDER " << s << " CENTROIDS " << std::endl;
	n1.first.order_centroids( );
	n1.second.order_centroids();

	richanalysis::node_analysis onode( n1, 0 );
	particles o_aligned = onode.ordered_fit();
	fIO.output_pdb("ordered" + std::to_string(s)
			 + opts[2].second , o_aligned ); 
